...
const SubcomponentsPage = () => {
    ...
    useEffect(() => {
        const fetchComponentName = async () => {
            try {
                const response = await axios.get(`http://localhost:5000/api/component/name/${componentId}`);
    ...
    useEffect(() => {
        const fetchSubComponents = async () => {
            try {
                const response = await axios.get(`http://localhost:5000/api/subComponent/${componentId}`);
    ...
    //     }
    //     try {
    //         const response = await axios.put(`http://localhost:5000/api/subComponent/update/${subComponentId}`, formData, {
    //             headers: {
    ...
    const handleDelete = async (subComponentId) => {
        try {
            await axios.delete(`http://localhost:5000/api/subcomponent/${subComponentId}`);
    ...
    //     try {
    //         const response = await axios.post(`http://localhost:5000/api/subComponent/create/${componentId}`, formData, {
    //             headers: {
    //                 'Content-Type': 'multipart/form-data',
    //             },
    //         });
    ...
    const handleNewSubComponentSave = async () => {
        const formData = new FormData();
        formData.append('title', newSubComponent.title);
        formData.append('text', newSubComponent.text);
        formData.append('spanText', newSubComponent.spanText);
        formData.append('description', newSubComponent.description);
        formData.append('buttonText', newSubComponent.buttonText);
        formData.append('htmlContent', newSubComponent.htmlContent);
                            ...
                            <li key={subComponent._id}>
                                {editingSubComponentId === subComponent._id ? (
                                    ...
                                    <div >
                                        ...
                                        <p><strong style={{ fontSize: "20px" }}>Text:</strong> <span dangerouslySetInnerHTML={{ __html: subComponent.text || 'null' }} /></p>
                                        ...
                                        <p><strong>Image:</strong>
                                            {subComponent.image ? (
                                                // <img src={subComponent.image}  style={{ maxWidth: '200px', maxHeight: '200px' }} />
                                                <img src={`http://localhost:5000/api/subcomponent/images/${subComponent.image}`} style={{ maxWidth: '200px', maxHeight: '200px' }} />
